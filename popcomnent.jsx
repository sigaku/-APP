...
import shoppingImage from '../app.UI/shopping.png';
...
import transferImage from '../app.UI/transfer2.png';

const Categories = [
  { id: 1, name: 'life', image: lifeImage },
  { id: 2, name: 'shopping', image: shoppingImage },
  { id: 3, name: 'transport', image: transportImage },
  { id: 4, name: 'food', image: foodImage },
  { id: 5, name: 'entertainment', image: entertainmentImage },
  { id: 6, name: 'transfer', image: transferImage },
];
...
const PopupComponent = ({ visible, onClose, onAmountEntered, onDataSent }) => {
  ...
  const [username, setUsername] = useState('123456'); // 新增状态，固定为 '12346'
  const [transactionType, setTransactionType] = useState('支出'); // 新增状态，默认为 '支出'
  ...
  const handleTransactionType = (type) => {
    ...
    if (type === '支出') {
      setAmount((prevAmount) => {
        const numericAmount = parseFloat(prevAmount);
  ...
  const handleSubmit = async () => {
    ...
    const data = {
      ...
      username: '123456', // 确保 username 被包含在数据中
      transactionType: transactionType, // 新增字段
    };
      ...
      <View style={styles.transactionTypeContainer}>
        <TouchableOpacity
          style={[
            ...
            transactionType === '支出' && styles.transactionTypeButtonSelected,
          ]}
          onPress={() => handleTransactionType('支出')}
          accessibilityLabel="选择支出"
          accessibilityHint="点击以选择支出"
        >
          <Text style={styles.transactionTypeText}>支出</Text>
        </TouchableOpacity>
        <TouchableOpacity
          style={[
            styles.transactionTypeButton,
            transactionType === '收入' && styles.transactionTypeButtonSelected,
          ]}
          onPress={() => handleTransactionType('收入')}
          accessibilityLabel="选择收入"
          accessibilityHint="点击以选择收入"
        >
          <Text style={styles.transactionTypeText}>收入</Text>
        </TouchableOpacity>
      </View>
      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          <TouchableOpacity
            ...
            style={[
              styles.categoryItem,
              category && category.id === item.id && styles.categoryItemSelected,
            ]}
